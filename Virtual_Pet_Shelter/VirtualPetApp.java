...
public class VirtualPetApp {
    public static void main(String[] args) {
        ...
        String petName = input.nextLine().trim(); // Trim added to remove spaces from input, avoids issues later in
                                                  // calling names for play and adoption
        System.out.println("What kind of pet is " + petName + ".");
        String petDescription = input.nextLine();
        // Here, I give the player the opportunity to tell me the pets custom stats.
        // Because Sometimes, Karen just KNOWS how fluffy is feeling
        // This idea brought to you by my 'caring and concerned girlfriend of the pets
        // being abandoned'
        // Hunger, thirst, and bored are given ratings of 1-10, and only accepted if
        // given a valid value in that range
        System.out.println(
        ...
        if (entryChoice.equalsIgnoreCase("y")) {
            while (!validHunger) {
                System.out
                        .println("Since you last fed " + petName
                                + " how hungry do you feel they are,\n on a scale of 1-10, with 10 being extremely hungry.");
            ...
            while (!validThirst) {
                System.out
                        .println("Since you last hydrated " + petName
                                + " how thirsty do you feel they are,\n on a scale of 1-10, with 10 being extremely thirsty.");
            ...
            while (!validBoredom) {
                System.out
                        .println("Since you last played with " + petName
                                + " how bored do you feel they are,\n on a scale of 1-10, with 10 being extremely bored.");
            ...
            // If 'N' was chosen, pet is created with base stats defined in VirtualPet.Java
            // Hunger=3, Thirst=2, Boredom=1
            VirtualPetShelter.put(petName,
        ...
        while (!menuChoice.equals("6")) {
            ...
            if (menuChoice.equals("1")) {
                ...
                System.out.println(petChoice + " had so much fun!");
            } else if (menuChoice.equalsIgnoreCase("4")) { // Like above, when adopting a new pet, will give the user
                                                           // the choice to bring in custom stats
                System.out.println("What is the pet's name?");
                petName = input.nextLine().trim();
                while (petShelter.nameCheck(petName)) { // Only unique names in my menagerie!! make the user change name
                                                        // before dropping off
                    System.out.println(
                            "We already have a pet on site by that name,\n please choose another name for your pet");
                    petName = input.nextLine().trim();
                }
                System.out.println("what kind of pet is " + petName + ".");
                petDescription = input.nextLine();
                System.out.println("Would you like to tell us " + petName
            ...
            } else if (menuChoice.equalsIgnoreCase("5")) { // Get list of pets on site to adopt out and give a forever
                                                           // home
                System.out.println("These are the current misfits in the menagerie\n");
                System.out.println(petShelter.petList());
                System.out.println("Which pet has found a forever home?");
                String petChoice = input.nextLine(); // can't figure out how to make this not care about case sensitive,
                                                     // in addition of name check
                while (!petShelter.nameCheck(petChoice)) {
                    System.out.println(
